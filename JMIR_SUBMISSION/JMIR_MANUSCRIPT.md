# MANUSCRIPT FOR JMIR MEDICAL INFORMATICS
...
**Objective:** To systematically evaluate consistency and accuracy of state-of-the-art LLMs in binary medical diagnosis, examining the relationship between reproducibility and diagnostic performance, and to provide practical guidance for clinical implementation.
...
**Conclusions:** Our findings reveal a critical dissociation between consistency and accuracy in LLM medical diagnosis. While LLMs demonstrate remarkable reproducibility, their tendency toward over-diagnosis and limited discriminative accuracy constrain direct clinical utility. For healthcare systems considering LLM deployment, we recommend restricting use to supplementary decision-support roles with mandatory human oversight, implementing rigorous validation protocols, and establishing clear guidelines for appropriate clinical contexts.

**Keywords:** artificial intelligence; large language models; clinical decision support; diagnostic accuracy; reproducibility; medical informatics; heart disease
...
Despite growing literature on LLM performance in medical question-answering [5,6], few studies have systematically examined the relationship between consistency and accuracy in diagnostic tasks. Most evaluations focus on single-run accuracy without assessing reproducibility, and the influence of prompt engineering remains incompletely characterized [7,11]. This gap is particularly problematic for clinical implementation, where understanding both diagnostic accuracy and reproducibility is essential for safety and reliability.

### Objectives

This study addresses these gaps through comprehensive evaluation of three state-of-the-art LLMs on binary heart disease diagnosis. Our aims were to: (1) quantify intra-model consistency across repeated runs, (2) evaluate inter-model agreement, (3) measure diagnostic accuracy relative to consistency, (4) assess prompt sensitivity, (5) analyze error patterns to determine if mistakes are random or systematic, and (6) provide evidence-based recommendations for clinical implementation.
...
Statistical analyses used Python with pandas, scikit-learn, and scipy. Significance was set at P<.05.

### Ethical Considerations

This study used publicly available, de-identified data from the UCI Machine Learning Repository. No human subjects were involved, and institutional review board approval was not required.
...
*Note: Consistency calculated as proportion of 4 independent runs with majority agreement. Perfect agreement indicates all 4 runs yielded identical predictions. See Multimedia Appendix 1 Table S1 for complete performance metrics and Table S3 for detailed consistency patterns.*

### Inter-Model Agreement: High Consensus

Models showed 98-100% pairwise agreement, indicating remarkably similar reasoning patterns (Table 2, Multimedia Appendix 1 Table S4). Three-way agreement (all models concur) occurred in 98-99% of cases. Cohen's kappa values near zero reflected extreme class imbalance (nearly all positive predictions) rather than lack of agreement.
...
*Note: Metrics calculated using majority voting (≥2/4 runs). Baseline prevalence: 51% positive. See Multimedia Appendix 1 Table S2 for representative sample predictions with justifications and Table S1 for complete statistical analysis.*
...
Changing from expert to neutral prompt had minimal effect (Multimedia Appendix 1 Table S5, Figure 3). GPT-4o showed zero sensitivity (100% identical predictions), while Gemini and Qwen changed only 1-3 predictions (1-3% of cases). This suggests diagnostic behavior is deeply encoded rather than easily modifiable through prompting.
...
*Note: "All Models Correct" indicates cases where all three models provided accurate diagnosis. "All Models Wrong" indicates shared errors. "Mixed Results" indicates disagreement among models. See Multimedia Appendix 1 Table S5 for prompt robustness analysis.*

Qualitative analysis (Multimedia Appendix 1 Table S2, Table S4) revealed models consistently cited elevated cholesterol, abnormal ECG findings, or exercise abnormalities as disease evidence, even when ground truth indicated absence of significant stenosis, suggesting risk factor conflation with diagnostic criteria.
...
This study demonstrates a critical dissociation between consistency and accuracy in LLM medical diagnosis: exceptional reproducibility (99-100%) coexists with chance-level accuracy (~50%). This 50-percentage-point gap represents a fundamental challenge for clinical deployment and has important implications for healthcare systems considering LLM implementation.
...
Current LLMs are not ready for primary diagnostic applications requiring binary classification. The ~50% accuracy is unacceptable clinically and could lead to harmful over-diagnosis, unnecessary testing, and patient anxiety [14]. In a 50% prevalence scenario, deploying these models would result in 98-100% of disease cases correctly identified but only 0-2% of healthy cases correctly identified, causing approximately 50% unnecessary downstream testing.

**Practical Recommendations for Healthcare Systems:**

1. **Appropriate Use Cases**: Restrict LLM deployment to supplementary roles (second opinion, triage assistance, medical education) rather than primary diagnosis
2. **Mandatory Human Oversight**: Require physician review of all LLM-generated diagnoses before clinical action
3. **Validation Protocols**: Establish rigorous local validation using institution-specific patient populations before deployment
4. **Transparency Requirements**: Clearly communicate LLM limitations to clinicians and patients
5. **Monitoring Systems**: Implement continuous performance monitoring to detect accuracy drift
6. **Bias Mitigation**: Screen for and address systematic over-diagnosis tendencies in clinical workflows

**Cost-Benefit Considerations:**

Despite limitations, LLMs' high consistency offers potential value in specific contexts:
- **Triage Screening**: High sensitivity (98-100%) suitable for initial patient stratification where missing cases is costly
- **Education**: Consistent feedback for medical students learning diagnostic reasoning
- **Research**: Hypothesis generation and systematic case review
- **Second Opinion**: Reproducible consultation where human expertise remains primary decision-maker

However, the 50% false positive rate would generate substantial downstream costs through unnecessary imaging, laboratory testing, and specialist referrals. Healthcare systems must weigh these costs against potential benefits in specific implementation contexts.

### Policy and Regulatory Implications

Our findings have important implications for AI governance in healthcare:

**Regulatory Frameworks**: Current LLM performance suggests need for stringent regulatory oversight before clinical deployment. Regulatory bodies (FDA, EMA, national agencies) should consider establishing specific approval pathways for diagnostic AI that require:
- Demonstration of both accuracy AND consistency metrics
- Local validation on institution-specific populations
- Transparent reporting of false positive/negative rates
- Post-market surveillance for performance drift

**Liability and Accountability**: The systematic nature of LLM errors (98-99% inter-model agreement on mistakes) raises questions about liability when multiple AI systems make identical incorrect diagnoses. Clear frameworks are needed to delineate responsibility between AI developers, healthcare institutions, and clinicians.

**Transparency Mandates**: Patients have right to know when AI systems contribute to diagnostic decisions. Policies should require disclosure of AI involvement, its known limitations (e.g., 50% false positive rate), and availability of human oversight.

**Quality Standards**: Healthcare accreditation bodies should develop minimum performance standards for clinical AI deployment, including requirements for ongoing validation, bias monitoring, and clinician training on appropriate AI use.

These findings underscore the need for evidence-based policy development that balances innovation with patient safety, particularly given the consistency-accuracy dissociation demonstrated in current LLM systems.

### Technical Implications for AI Development

Results suggest general-purpose LLMs lack discriminative capabilities for diagnostic classification. Future development should consider:

1. **Supervised Fine-Tuning**: Training on labeled diagnostic datasets with explicit outcome labels
2. **Reinforcement Learning**: Optimization based on physician-verified diagnoses
3. **Calibration Techniques**: Post-processing to adjust decision thresholds for binary classification
4. **Hybrid Architectures**: Combining LLM reasoning with specialized diagnostic classifiers
5. **Uncertainty Quantification**: Explicit confidence scoring to flag ambiguous cases
6. **Feedback Mechanisms**: Incorporating clinician corrections into model updates

### Comparison with Prior Work

Our consistency findings (99-100%) align with recent reports of high LLM reproducibility in structured tasks [15], while our accuracy findings (~50%) contrast with higher reported accuracies in medical question-answering tasks [1-3]. This discrepancy likely reflects differences between knowledge recall (where LLMs excel) and discriminative diagnosis (where they struggle), highlighting the importance of task-specific evaluation before clinical deployment.
...
Future studies should evaluate performance across diverse diseases, clinical narratives, larger patient cohorts, and varied model architectures to establish generalizability.

### Future Directions

Important future work includes:

1. **Mechanistic Studies**: Examining which clinical parameters LLMs prioritize and why
2. **Improvement Strategies**: Testing fine-tuning, calibration, and ensemble approaches
3. **Broader Evaluations**: Assessing performance across diverse diagnostic tasks and specialties
4. **Human Comparison**: Benchmarking against physician performance for baseline context
5. **Implementation Research**: Studying real-world integration challenges and workflow impacts
6. **Theoretical Development**: Building consistency-accuracy frameworks for AI diagnostics

---

## CONCLUSIONS

This study provides rigorous evidence that LLMs achieve exceptional consistency (99-100%) but limited accuracy (~50%) in binary medical diagnosis. This consistency-accuracy dissociation represents a fundamental challenge for clinical deployment. Our key findings indicate: (1) high consistency does not guarantee accuracy, (2) diagnostic behavior is resistant to prompt engineering, (3) errors are systematic rather than random, and (4) LLMs show strong positive diagnosis bias.

For healthcare systems considering LLM deployment, we recommend restricting use to supplementary decision-support roles with mandatory human oversight, implementing rigorous local validation protocols, and establishing clear guidelines for appropriate clinical contexts. Current general-purpose LLMs are better suited as assistive tools rather than primary diagnostic systems. Their exceptional reproducibility is clinically valuable, but limited discriminative ability necessitates careful integration into clinical workflows with ongoing performance monitoring.

Future development should prioritize supervised fine-tuning on diagnostic datasets, improved structured data processing, and calibration techniques to address systematic biases. This work contributes to evidence-based understanding of LLM capabilities and limitations in healthcare, informing responsible development and deployment of AI-assisted clinical decision support systems.
